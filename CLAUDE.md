...
- **ヘッダーのモバイル表示は`.site-nav a:nth-child(n + 3)`で3つ目以降
  (キャンプ/About)を隠して2項目だけにしている(幅が足りないため)。**
  新しいリンクを追加するときにナビへ足すと、この間引きでさらに隠れてしまう。
  常時見せたいリンク(例: SUZURIのショップへのカートアイコン)は`.site-nav`の
  外側、`.site-header__right`直下に置いて間引きの対象から外す。
