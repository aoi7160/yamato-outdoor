...
記事の本文はmicroCMSに入るので、Gitでの作業は発生しない。**ただし、その記事を書く前に
決めたこと(ペルソナ・SERP調査・見出し構成の根拠)は `docs/articles/<スラッグ>.md` として
Gitに残す。** 詳しくは `docs/articles/README.md`。チャットのセッションは引き継がれないので、
ここに書いておかないと次にこの記事を触る人(未来の自分を含む)が同じ調査をやり直すことになる。
...
8. `docs/articles/<スラッグ>.md` を作成(または更新)してコミット・push する
   (作業ブランチが無ければ `claude/article-notes-<日付>` のような軽いブランチで良い)

### 本文は2通り。リッチエディタか、HTML入稿か

**microCMSのリッチエディタにHTMLを貼っても効きません。** 入力してもエスケープされ、
タグが文字としてそのまま表示されます。「HTML編集」のようなボタンもありません。
そのため本文の入れ物を2つ用意してあります。

| フィールド | 種類 | 使いどころ |
|---|---|---|
| `content` | リッチエディタ | 見出し・段落・リスト・表・画像だけで足りる普通の記事 |
| `contentHtml` | テキストエリア | 下の装飾クラスを使いたい記事。**HTMLをそのまま貼る** |

`contentHtml` に何か入っていれば、そちらが本文として使われる(`pickContent()`)。
両方に書くと `contentHtml` が勝つので、**使わないほうは空にしておく**。

HTMLで入稿するときは、`<p>` や `<h2>` から始まる本文をそのまま貼り付ければよい。
改行やインデントは表示に影響しない。

### 記事内で使える装飾(`contentHtml` に貼る)
