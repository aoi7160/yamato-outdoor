...
| content | 本文 | リッチエディタ | 普通の記事はこちら |
| contentHtml | 本文(HTML) | テキストエリア | 任意。HTMLで入稿する記事だけ使う。下記参照 |
...
### 本文フィールド(content / contentHtml)について

**microCMSのリッチエディタにHTMLを貼っても効かない。** 入力してもエスケープされ、
タグが文字としてそのまま出てしまう。「HTML編集」のようなボタンも無い。

そのため本文の入れ物を2つ用意している。

- `content`(リッチエディタ): 見出し・段落・リスト・表・画像で足りる普通の記事
- `contentHtml`(**テキストエリア**): `note` や `checklist` などの装飾クラスを使う記事。
  HTMLをそのまま貼る

`contentHtml` に中身があればそちらが本文になる(`pickContent()` in `src/lib/article.ts`)。
**両方に書かないこと。** 使わないほうは空にしておく。

テキストエリアは文字数制限を付けずに作る(長い記事が入らなくなるため)。
